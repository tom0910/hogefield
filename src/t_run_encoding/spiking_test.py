...
from display.tk_widgets_setup import create_widgets, initialize_plots, create_plots_frame_simple2
...
if __name__ == "__main__":
    ...
    # create plots
    widget_frame, out_frame_audio_wave_plt,  out_spike_raster_plt = create_plots_frame_simple2(root)
    ...
    def update_plot():
        ...
        time_plt = time[0:1000]
        amplitude_plt = amplitude[0:1000]
        print(f"amplitude: type={type(amplitude)}, shape={getattr(amplitude, 'shape', 'N/A')}")
        plot_waveform(time_plt, amplitude_plt, canvases[0], axes[0])
    ...
    def update_plot_spike():
        ...
        #calculation:
        num_neurons, num_spike_index, spikes, _, _ = FU.generate_spikes_of_audio(audio_sample, mel_config, spike_threshold_updated, norm_inp=True, norm_cumsum=False)
        print("spikes calculated")
        # plot spike trains or distribution
        if plot_choice == C.DEFAULT_SPIKE_PLT_PICK:
            #plot:
            plot_spikes(spikes=spikes, num_neurons=num_neurons, num_spike_index=num_spike_index, canvas=canvases[1], ax=axes[1])
    ...
    threshold_slider.set(0.00023)
    threshold_var=tk.DoubleVar(value=0.00023)
    threshold_entry.configure(textvariable=threshold_var)
