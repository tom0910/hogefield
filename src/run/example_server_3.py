...
def open_browser_in_tkinter():
    ...
    # Load the Flask URL (audio player)
    browser.load_url("http://localhost:5000")
    
    # Start the Tkinter event loop
    root.mainloop()

def display_audio_in_widget(waveform, sample_rate, output_frame):
    """
    ...
    # Open the browser in Tkinter to display the audio player
    open_browser_in_tkinter()

# Test with waveform and sample_rate
waveform, sample_rate = audio_sample.load_waveform()
display_audio_in_widget(waveform, sample_rate, None)  # None as output_frame for now
